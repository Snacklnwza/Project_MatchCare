...
## 13. แสดงอายุจากวันเกิด

วันเกิดยังเก็บเป็น `birth_date` ในฐานข้อมูล ไม่บันทึกอายุคงที่ เพราะอายุเปลี่ยนทุกปี `src/lib/patients.js` คำนวณอายุเต็มปีตามวันที่ปัจจุบันในเขตเวลา Asia/Bangkok แล้วแสดงในรายการผู้ป่วย หน้าแรกผู้ว่าจ้าง และตัวเลือกผู้ป่วยของฟอร์มประกาศ ทดสอบขอบเขตก่อน/ตรงวันเกิด ปีอธิกสุรทิน วันเกิดในอนาคตและวันที่ไม่ถูกต้อง รวมถึงตรวจการแสดงผลทั้งสามจุดด้วยข้อมูลจำลองในเบราว์เซอร์

## 14. ทดสอบหลังรวม SQL เป็น 5 ไฟล์ (26 กันยายน 2026)

เทียบชุดก่อนรวมที่ commit `e57aac6` กับชุดใหม่ `a7e484f`: คำสั่งจากทั้ง 17 ไฟล์เดิมตรงกันครบทุกช่วง จากนั้นสร้างฐานว่างสองฐานด้วย PGlite 0.5.8 และรันแต่ละชุดตามลำดับของชุดนั้นจริง ทั้งสองชุดติดตั้งสำเร็จ

ตรวจ catalog แล้วตรงกัน: 10 ตารางและ 4 sequences, 83 คอลัมน์, constraints, 26 RLS policies, 11 functions, 12 triggers, 24 indexes, สิทธิ์ระดับ schema/table/column/function และข้อมูลตั้งต้น skills/conditions

- ชุด `sprint1_integration_test.sql` ผ่านทั้งฐานเก่าและฐานใหม่: บัญชี/บทบาท, กฎผู้ป่วย, สร้าง–แก้–ปิดงาน, rollback เมื่อข้อมูลผิด, ป้องกันข้ามบัญชี และปฏิเสธ anon
- ทดสอบบันทึกผู้ป่วยด้วย `normal` ผ่าน RPC และใช้กับประกาศงานสำเร็จ
- จำลองฐานเดิมที่ยังไม่มี `normal` แล้วรัน migration รวมถึงรันซ้ำ ผ่านและได้ constraint ตรงกับฐานใหม่
- รัน integration บน Supabase หลักด้วยข้อมูลชั่วคราว ได้ `true` ทั้ง 6 ค่า และจบด้วย `ROLLBACK` (sequence อาจมีเลขข้ามตามพฤติกรรม PostgreSQL)

พบข้อผิดพลาดใน test เดิม: `current_date + 1` ใช้วัน UTC ซึ่งช่วง 00:00–06:59 เวลาไทยอาจเท่ากับวันนี้ของไทย จึงแก้ test วันเกิดทั้งสองไฟล์ให้ใช้ `(now() at time zone 'Asia/Bangkok')::date + 1` พร้อมตรวจว่าวันนี้ตามเวลาไทยยังบันทึกได้ แล้วรัน integration ซ้ำผ่านทั้งหมด การแก้ครั้งนี้อยู่ใน SQL tests ไม่เปลี่ยนกฎของแอป

ขอบเขต: ฐานว่างใช้ [PGlite](https://pglite.dev/docs/api) พร้อมจำลองเฉพาะ `auth.users` และ `auth.uid()` ที่ SQL ต้องใช้ จึงไม่ใช่การทดสอบบริการส่งอีเมลหรือหน้าเว็บครบทุกหน้า ผลข้างต้นรับรองการจัดไฟล์ SQL และ flow ที่ชุดทดสอบครอบคลุม

## 15. ตรวจเว็บ Vercel จริง (26 กันยายน 2026)

ทดสอบ `https://matchcare-chi.vercel.app/` ด้วย Playwright/Chrome และบัญชีทดสอบ:

- ฟอร์มสมัคร: ต้องยอมรับเงื่อนไขก่อนส่ง และรหัสผ่านยืนยันไม่ตรงกันถูกปฏิเสธ
- เข้าสู่ระบบ Employer, Caregiver และ Admin ไปยังหน้าตามบทบาท; รหัสผ่านผิดถูกปฏิเสธ; session ของ Employer/Caregiver ยังใช้ได้หลัง reload; ออกจากระบบได้
- ผู้ป่วย: ตรวจข้อมูลว่าง/วันเกิดอนาคต, เลือกพะเยา–อำเภอ–ตำบล, เปลี่ยนจังหวัดแล้วล้างค่าลูก, บันทึก `normal`, แสดงอายุ, แก้วันเกิด/ที่อยู่และ reload แล้วยังได้ข้อมูลล่าสุด
- ประกาศ: เลือกผู้ป่วยพร้อมอายุ, ปฏิเสธการไม่เลือกทักษะ/วันสิ้นสุดก่อนวันเริ่ม/ค่าตอบแทน 0, สร้างจริง, แก้ค่าตอบแทนและทักษะ, เวลาเริ่มงานคงเดิมในหน้าแก้ไข, dashboard แสดงงานที่เปิดจริง
- ปิดใช้งาน: ผู้ป่วยที่มีงานเปิดถูกปฏิเสธพร้อมเหตุผล, ยกเลิกกล่องยืนยันแล้วข้อมูลยังอยู่, ยืนยันปิดงานแล้วหายจากตัวกรองเปิดรับ, จากนั้นปิดผู้ป่วยสำเร็จ
- ตรวจหน้าฟอร์มสมัคร ผู้ป่วย และประกาศที่ความกว้าง 390px ไม่ล้นแนวนอน พร้อมภาพ desktop/mobile ใน `output/playwright/live-*.png`

รายการสมมติของรอบนี้คือ patient ID 37 และ job ID 12: ตรวจฐานข้อมูลหลังจบแล้วผู้ป่วย `is_active=false`, งาน `closed`, ค่าตอบแทนที่แก้เป็น 1500 บาท รายการยังเก็บประวัติตามระบบ soft delete

ไม่พบข้อผิดพลาดของแอปใน flow ที่ทดสอบ HTTP 400 ของการปิดผู้ป่วยขณะงานเปิดและการเข้าระบบด้วยรหัสผิดเป็นกรณีปฏิเสธที่ตั้งใจทดสอบ ระหว่างรันทดสอบมีการแก้ชื่อปุ่ม/วิธีตรวจ native validation ใน test harness และเปิด browser session ใหม่ ไม่มีการแก้โค้ด production

ข้อจำกัด: รอบนี้ไม่ได้สมัครบัญชีใหม่แล้วรับอีเมลจริง ไม่ได้เทียบทุกหน้ากับ Figma และไม่ได้ใช้หน้า placeholder ของ Sprint 2/3 เป็นหลักฐานว่าฟังก์ชันเหล่านั้นเสร็จ
