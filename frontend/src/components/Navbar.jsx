...
function Navbar({ profile, activePage, onSelect, onSignOut }) {
    ...
    const roleLabel = { employer: 'ผู้ว่าจ้าง', caregiver: 'ผู้ดูแล', admin: 'ผู้ดูแลระบบ' }[role]

    return (
      <header className="top-navbar">
        <div className="top-navbar-inner">
          <button type="button" className="navbar-brand" onClick={() => onSelect('dashboard')} aria-label="MatchCare หน้าหลัก">
        ...
        <nav className="role-navigation" aria-label="เมนูหลัก">
          ...
          <div className="navbar-account">
            <details className="navbar-dropdown">
              <summary className="notification-toggle" aria-label="การแจ้งเตือน"><img src={bell} alt="" /></summary>
              <div className="navbar-popover"><strong>การแจ้งเตือน</strong><p>ระบบแจ้งเตือนยังไม่เปิดใช้งาน</p></div>
            </details>
            <details className="navbar-dropdown">
              <summary className="account-toggle">
                <span className="account-name">คุณ{profile.first_name || 'ผู้ใช้งาน'}</span>
                <span className="account-role">({roleLabel})</span>
                <img src={chevron} alt="" />
              </summary>
              <div className="navbar-popover"><button type="button" onClick={onSignOut}>ออกจากระบบ</button></div>
            </details>
          </div>
        </div>
      </header>
